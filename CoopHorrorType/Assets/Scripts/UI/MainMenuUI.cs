...
public class MainMenuUI : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            LobbyNetworkManager.Instance.OnPlayerListUpdated += UpdatePlayerListUI;
        }
    }
    ...
    private async void OnCreateHostClicked()
    {
        ...
        if (LobbyNetworkManager.Instance != null)
        {
            LobbyNetworkManager.Instance.SetupConnectionApproval(localName);
            LobbyNetworkManager.Instance.RegisterCallbacks();
        }
        ...
        if (!string.IsNullOrEmpty(code))
        {
            ...
            {
                VoiceChatManager.Instance.Join3DVoiceChannel(code);
            }
            ...
            if (LobbyNetworkManager.Instance != null)
            {
                LobbyNetworkManager.Instance.RegisterHostName(localName);
            }
            UpdatePlayerListUI();
        }
        else
    ...
    {
        if (playerListText != null && LobbyNetworkManager.Instance != null)
        {
            playerListText.text = LobbyNetworkManager.Instance.FormatPlayerListText();
        }
    }
    ...
    private void OnStartGameClicked()
    {
        if (LobbyNetworkManager.Instance != null)
        {
            LobbyNetworkManager.Instance.StartGameSequence();
        }
    }
}